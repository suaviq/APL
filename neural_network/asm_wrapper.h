...
private:
	...
	// DLL Procedures
	f_asm_f64x3 add_vectors;
	f_asm_f64x3 subtract_vectors;
	f_asm_f64x3 add_scalar_to_vector;
	f_asm_f64x3 subtract_scalar_from_vector;
	f_asm_f64x3 vector_wise_multiply;
	f_asm_f64x3 mul_vecT_by_vec;
	f_asm_f64x3 mul_vec_by_vecT;
	f_asm_f64x3 mul_vector_by_scalar;
	f_asm_f64x3 mul_matrix_by_scalar;
	f_asm_f64x3 add_matrices;
	f_asm_f64x3 subtract_matrices;
...
AsmWrapper::AsmWrapper() {
	...
	subtract_vectors = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "subtract_vectors");
	if (!subtract_vectors) {
		std::cerr << "[ERROR] could not locate the function `subtract_vectors`" << std::endl;
		exit( EXIT_FAILURE );
	}
	add_vectors = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "add_vectors");
	if (!add_vectors) {
		std::cerr << "[ERROR] could not locate the function `add_vectors`" << std::endl;
		exit( EXIT_FAILURE );
	}
	add_scalar_to_vector = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "add_scalar_to_vector");
	if (!add_scalar_to_vector) {
		std::cerr << "[ERROR] could not locate the function `add_scalar_to_vector`" << std::endl;
		exit( EXIT_FAILURE );
	}
	subtract_scalar_from_vector = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "subtract_scalar_from_vector");
	if (!subtract_scalar_from_vector) {
		std::cerr << "[ERROR] could not locate the function `subtract_scalar_from_vector`" << std::endl;
		exit( EXIT_FAILURE );
	}
	mul_vector_by_scalar = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "mul_vector_by_scalar");
	if (!mul_vector_by_scalar) {
		std::cerr << "[ERROR] could not locate the function `mul_vector_by_scalar`" << std::endl;
		exit( EXIT_FAILURE );
	}
	vector_wise_multiply = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "vector_wise_multiply");
	...
	add_matrices = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "add_matrices");
	if (!add_matrices) {
		std::cerr << "[ERROR] could not locate the function `add_matrices`" << std::endl;
		exit( EXIT_FAILURE );
	}
	subtract_matrices = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "subtract_matrices");
	if (!subtract_matrices) {
		std::cerr << "[ERROR] could not locate the function `subtract_matrices`" << std::endl;
		exit( EXIT_FAILURE );
	}
	mul_matrix_by_scalar = (f_asm_f64x3)GetProcAddress(hGetProcIDDLL, "mul_matrix_by_scalar");
