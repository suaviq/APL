...
public:
	...
	inline double  mul_vec_by_vecT(double* v, double* vT) { double x = double(0); return x; }
...
CppWrapper::CppWrapper() {
	...
	_mul_vector_by_scalar = (f_dll_f64x3)GetProcAddress(hGetProcIDDLL, "mul_vector_by_scalar_cpp");
	if (!_mul_vector_by_scalar) {
		std::cerr << "[ERROR] could not locate the function `mul_vector_by_scalar_cpp`" << std::endl;
		exit( EXIT_FAILURE );
	}
	_vector_wise_multiply = (f_dll_f64x3)GetProcAddress(hGetProcIDDLL, "vector_wise_multiply_cpp");
	if (!_vector_wise_multiply) {
		std::cerr << "[ERROR] could not locate the function `vector_wise_multiply_cpp`" << std::endl;
		exit( EXIT_FAILURE );
	}
	_mul_vecT_by_vec = (f_dll_f64x3)GetProcAddress(hGetProcIDDLL, "mul_vecT_by_vec_cpp");
	if (!_mul_vecT_by_vec) {
	...
	_relu_vec = (f_dll_f64x2)GetProcAddress(hGetProcIDDLL, "relu_vec_cpp");
	if (!_relu_vec) {
		std::cerr << "[ERROR] could not locate the function `relu_vec_cpp`" << std::endl;
		exit( EXIT_FAILURE );
	}
	_derivative_relu_vec = (f_dll_f64x3)GetProcAddress(hGetProcIDDLL, "derivative_relu_vec_cpp");
	...
	_mul_matrix_by_scalar = (f_dll_f64x3)GetProcAddress(hGetProcIDDLL, "mul_matrix_by_scalar_cpp");
